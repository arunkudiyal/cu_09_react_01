...
const App = (props) => {
  ...
  const onShowPersons = () => {
    let stateValue = personState.showPersons
    setPersonState({ showPersons: !stateValue, persons: personState.persons })
  }

  const deletePersonHandler = (index) => {
    let personsCopy = [personState];
    // If ID is passed
    // personsCopy.filter(person => person.id !== id)
    personsCopy.splice(index, 1)
    setPersonState( {persons: personsCopy} )
  }
  ...
  return (
    <div>
        <div className="container-fluid">
          <h1 style={{ color: 'red', fontSize: '2.2em' }}> {props.appTitle} </h1>
          <button onClick={onShowPersons} className='btn btn-success'>Toggle Persons</button>
          <hr />
        </div>
        <div className='container'>
          { persons }
